...
<div class="flex flex-row gap-8 w-full">
        ...
        <div class="flex flex-col gap-1">
            ...
            <div class="inline-flex">
                {#each data.album.artists as artist, index}
                    <a href={resolveSpotifyUri(artist.uri)} class="text-sm text-gray-500 hover:text-gray-400">{artist.name}</a>
    ...
    >
        {#each data.album.tracks.items as track}
            <TrackItem {track} contextUri={data.album.uri} offset={track.uri} />
        {/each}
    </div>
</div>
