...
<button on:click={onMenuClick} bind:this={element} class="relative rounded-full h-8 w-8 {open ? "bg-gray-700" : "bg-gray-800"} font-mono flex items-center justify-center cursor-pointer border border-gray-700 hover:bg-gray-700">
    <span>{initial}</span>
    {#if open}
        <div class="absolute z-10 w-40 border border-gray-800/50 flex flex-col right-0 top-full bg-gray-900 rounded-md mt-2 cursor-default">
            <span class="text-sm w-full text-center border-b border-gray-800/50 py-2 text-gray-500">
                {username}
            </span>
            <div class="flex flex-col gap-1">
                <a data-sveltekit-preload-data="off" href="/logout" class="text-sm text-gray-500 hover:text-gray-400 py-2 text-right px-3">Logout</a>
            </div>
        </div>
    {/if}
</button>
